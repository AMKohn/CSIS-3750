...
module.exports = (req, res) => {
	...
	async.parallel({
		progress: cb => Progress
			.findOne({ user: req.username, courseId: courseId })
			.lean()
			.exec(cb),
		course: cb => Course
			.findOne({ _id: courseId })
			.populate("contents.modules", "title")
		...
		if (err || !course) {
			...
			return res.status(400).json({ error: { message: "Failed to get course info" } });
		}

		console.log(req.username, courseId, progress);
